...
function RoundedBoxGeometry( size, radius, radiusSegments ) {
  ...
  function doVertices() {
    ...
    for ( var y = 0; y <= 0 1 6 7 radiussegments; y ++ ) { var v="y" va="v" * pihalf; cosva="Math.cos(" ); sinva="Math.sin(" if ( radiussegments vertex.set( 0, 1, vert="vertex.clone().multiplyScalar(" radius ).add( corneroffset cornerverts[ ].push( vertexpool.push( norm="vertex.clone();" cornernormals[ normalpool.push( continue; } for x="0;" <="radiusSegments;" u="x" ha="u" vertex.x="cosVa" math.cos( vertex.y="sinVa;" vertex.z="cosVa" math.sin( i="1;" 8; j="0;" ].length; ][ ].clone().multiply( cornerlayout[ ] function docorners() flips="[" true, false, true ]; lastrowoffset="rs1" - i; 1; r1="v" rs1; r2="(" + u1="u" a="cornerOffset" u; b="cornerOffset" u1; c="cornerOffset" d="cornerOffset" ! flips[ indices.push( else lastvertex; dofaces() cornervertnumber; cornervertnumber 2; 3; 4; 5; 6; 7; doheightedges() coffset="i" crowoffset="4" coffset; needsflip="i" & dodepthedges() cstarts="[" 2, 4, cends="[" 3, 5, cstart="cornerVertNumber" cstarts[ cend="cornerVertNumber" cends[>= i;
  ...
  function doWidthEdges() {
    ...
    for ( var i = 0; i < 4; i ++ ) {
      ...
      for ( var u = 0; u <= 1 3 end; u ++ ) { var a="cStart" + radiussegments * rs1; b="cStart" ( !="end" ? rs1 : cornervertnumber - ); c="cEnd" d="cEnd" if needsflip[ i ] indices.push( } else index="0;" for < vertexpool.length; positions.setxyz( index, vertexpool[ ].x, ].y, ].z normals.setxyz( normalpool[ ++; this.setindex( new three.bufferattribute( uint16array( indices ), this.addattribute( 'position', positions 'normal', normals roundedboxgeometry.prototype="Object.create(" three.buffergeometry.prototype roundedboxgeometry.constructor="RoundedBoxGeometry;" function roundedplanegeometry( size, radius, depth x, y, width, height; x="y" = size 2; width="height" size; radius="size" radius; const shape="new" three.shape(); shape.moveto( y shape.lineto( height shape.quadraticcurveto( height, geometry="new" three.extrudebuffergeometry( shape, depth: depth, bevelenabled: false, curvesegments: return geometry; class cube constructor( game this.game="game;" this.geometry="{" piecesize: 3, piececornerradius: 0.12, edgecornerroundness: 0.15, edgescale: 0.82, edgedepth: 0.01, }; this.holder="new" three.object3d(); this.object="new" this.animator="new" this.holder.add( this.animator.add( this.cubes="[];" this.generatepositions(); this.generatemodel(); this.pieces.foreach( piece> {
...
class Tween extends Animation {
  ...
  update( delta ) {
    ...
    else if ( this.progress <= 1 ) this.onupdate( this ); else { this.progress="1;" this.value="1;" this.oncomplete( super.stop(); } updateyoyo() if (> 1 || this.progress < 0 ) {
...
class Scores {
  ...
  convertTime( time ) {
    ...
    if ( time <= 0 10 60 1000 ) return 0; const seconds="parseInt(" ( time % ); minutes="parseInt(" * + ':' < ? '0' : '' seconds; } class storage { constructor( game this.game="game;" init() this.loadgame(); this.loadpreferences(); loadgame() this.game.saved="false;" loadpreferences() this.game.controls.flipconfig="0;" this.game.scrambler.scramblelength="20;" this.game.world.fov="10;" this.game.world.resize(); this.game.themes.settheme( 'cube' false; themes this.theme="null;" this.colors="{" cube: u: 0xfff7ff, d: 0xffef48, f: 0xef3923, r: 0x41aac8, b: 0xff8c0a, l: 0x82ca38, p: 0x08101a, g: 0xd1d5db, }, erno: 0xffffff, 0xffd500, 0xc41e3a, 0x0051ba, 0xff5800, 0x009e60, 0x111111, 0x8abdff, dust: 0xfff6eb, 0xe7c48d, 0x8f253e, 0x607e69, 0xbe6f62, 0x849f5d, camo: 0xbfb672, 0x805831, 0x718456, 0x37241c, 0x37431d, rain: 0xfafaff, 0xedb92d, 0xce2135, 0x449a89, 0xec582f, 0xa3a947, 0x87b9ac, }; settheme( theme if return; colors="this.colors[" ]; this.game.cube.pieces.foreach( piece> {
...
window.game = new Game();</path></path></path></path></=></=></=></=>